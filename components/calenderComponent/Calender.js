...
const Portal = ({ title, date, handleDelete, handlePotalClose }) => {
  return (
    <PortalWrapper>
      ...
      <TrashIcon
        className="w-9 h-9 text-red-500 cursor-pointer transition-all hover:w-11 h-11"
  ...
  return (
    <ScheduleForm>
      ...
      <div className="flex justify-between items-center">
        <button
          onClick={handleClose}
          className="text-white bg-red-600 p-3 hover:bg-red-400 mb-0"
        >
          Cancel
        </button>
        <button
          onClick={handleSave}
          className="text-white bg-green-600 p-3 hover:bg-green-400 mb-0"
        >
          Save
        </button>
      </div>
