...
const ClassListById = ({ users }) => {
  ...
  const handleSelect = async (value) => {
    ...
    if (isSelected) {
      ...
      // Use selectedValues from state directly
      const updatedSelectedValues = [...selectedValues, value];
      setSelectedValues(updatedSelectedValues);
  
      // Use the updatedSelectedValues when calling getWeekDays
      try {
        const weekDates = await getWeekDays(startDate, endDate, updatedSelectedValues);
          ...
          for (let i = 0; i < weekDays.length; i++) {
            ...
            try {
              if (attendance_id.length !== 0) {
                ...
                if (data.success) {
                  handleSharePointBookingRoom([weekDays[i]], [attendance_id], 0);
                  deleteTable()
