 ...
 * File: pages\api\admin\account\verifiedUser.js
...
import { findUserData, UpdateIsVerified } from '../../controller/accountquery';
...
async function handler(req, res) {
  ...
  try {
    if (existingUserID.result) {
      const updatedUser = await UpdateIsVerified(connection, ID, isVerified);
      console.log(updatedUser);
      sis_app_logger.info(
        `${new Date()}=${user.role}=account unlocked=${user.email}=${
      ...
      res.status(200).json({
        message: 'This account has become active',
      });
    }
  } catch (error) {
    sis_app_logger.error(
      `${new Date()}=activate account error=${user.role}=${user.email}=${
        error.message
