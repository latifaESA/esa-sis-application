...
const excludedKeys = ["password", "isVerified", "major", "appisSaved", "isreset"];
...
// const headerRow = allKeys.filter(key =>key !="password" && key !== "isVerified" && key !== "major" && key !== "appisSaved" && key !== "isreset");
  ...
  const dataAll=[headerRow,...selectedUsers.map(user =>
    headerRow.map(key => (user[key]))
  )
];
...
FileSaver.saveAs(data,`Students_Data_On_${moment().format('DD-MM-YYYY')}${fileExtension}`)   


const reportData=new FormData()
reportData.append('file', data);
const appReportName=`Students_Data_Report_On_${moment().format('DD-MM-YYYY')}-admin-${session.user.name}.xlsx`
